const mamba_game = (function () {
  ...
  globalHighscoreDatabase.on("value", function (snap) {
    ...
    if (globalHighscores.length > 40) {
      globalHighscores.pop();
    }
  });
  ...
  if (storage.getItem("sound") == null) {
    ...
    setTimeout(function () {
      if (window.sound == 1) {
        document
          .querySelector(".toggle-sound")
          .classList.add("toggle-sound--on");
      } else {
        document
          .querySelector(".toggle-sound")
          .classList.add("toggle-sound--off");
      }
    }, 0);
  }
      ...
      if (positions.length >= wallThreshold) {
        ...
        tempArray.forEach(function (position) {
          newWallArray.push([
            ...
            position[1],
            "wall",
            random(1, maxWallLifespan),
          ]);
        });
        if (maxWallLifespan < 260) {
          maxWallLifespan += 4;
        }
  ...
  const bronze = (function () {
    ...
    function removeBronze(coordinate) {
      bronzePositions.forEach(function (bronzePosition, index) {
        if (
          bronzePosition[0] == coordinate[0] &&
          bronzePosition[1] == coordinate[1]
        ) {
          bronzePositions.splice(index, 1);
        }
      });
  ...
  function gameOver(positions, collisionPosition) {
    ...
    function isGlobalHighscore(globalHighscores, endScore) {
      ...
      if (
        (globalHighscores.length < 40 && endScore > 0) ||
        endScore > lowestHighscore
      ) {
        return true;
      } else {
        return false;
    ...
    function setLocalHighcores(highscoreString, name, endScore) {
      if (highscoreString == null) {
        ...
        processedHighscoreString =
          highscoreString + ";" + name + "-" + endScore;
      ...
      for (i = 0; i < 3; i++) {
        ...
        for (j = 0; j < 13; j++) {
          column.innerHTML +=
            ...
            place +
            '.</span><span class="name"></span></span><span class="score"></span></div>';
          place++;
        }
      }
    ...
    function showHighscores(type) {
      ...
      document.querySelector("." + type + "-highscores").style.display =
        "block";
    }
